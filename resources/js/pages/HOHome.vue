...
<template>
  <div class="flex h-screen bg-[#f0f7ff] font-sans overflow-hidden relative">
    ...
    <aside :class="['bg-white border-r border-gray-200 flex flex-col transition-all duration-300 z-10', isCollapsed ? 'w-20' : 'w-64']">
      <div class="p-6 flex flex-col items-center border-b border-gray-100 overflow-hidden">
        <div :class="['rounded-full border-[3px] border-[#00a6e6] flex items-center justify-center bg-white shadow-sm transition-all', isCollapsed ? 'w-10 h-10' : 'w-20 h-20 mb-2']">
          <span :class="['text-[#00a6e6] font-black', isCollapsed ? 'text-xl' : 'text-4xl']">B</span>
        </div>
        <h1 v-show="!isCollapsed" class="text-center text-[#00a6e6] font-bold leading-tight uppercase tracking-tight whitespace-nowrap">Rural bank <br/> of Paete, Inc.</h1>
      </div>

      <nav class="flex-1 mt-2">
        <div v-for="item in navItems" :key="item.name" 
          :class="['flex items-center py-3 cursor-pointer transition-colors', isCollapsed ? 'justify-center' : 'px-6', item.active ? 'bg-[#00a6e6] text-white' : 'text-gray-600 hover:bg-gray-50']">
    ...
    <main class="flex-1 flex flex-col min-w-0">
      <header class="m-4 bg-[#00a6e6] rounded-xl flex items-center justify-between px-6 py-3 text-white shadow-sm">
        <div class="flex items-center gap-4">
          <button @click="toggleSidebar" class="p-1 hover:bg-white/20 rounded-md transition-colors">
